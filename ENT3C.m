...
Similarity = [];
figure(1);hold on;tiledlayout('flow');hold on
...
for ChrNr=ChrNrs
    ...
    for f=1:size(comparisons,1)
        ...
        S1 = ENT3C_OUT(strcmp(ENT3C_OUT.Name,comparisons{f,1})&ENT3C_OUT.ChrNr==ChrNr,:);
        S2 = ENT3C_OUT(strcmp(ENT3C_OUT.Name,comparisons{f,2})&ENT3C_OUT.ChrNr==ChrNr,:);
        Q = corrcoef(S1.S,S2.S);Q=Q(1,2);

        Similarity=[Similarity;...
            table(ChrNr,comparisons{f,1},comparisons{f,2},Q,...
            'VariableNames',{'ChrNr','Sample1','Sample2','Q'})];

        if ~ismember(comparisons{f,1}, plotted)
            figure(1);plot(S1.S,'color',CM(c,:),'linewidth',1.3);hold on;axis tight
            plotted = [plotted;comparisons{f,1}];c=c+1;
        end
        if ~ismember(comparisons{f,2}, plotted)
            figure(1);plot(S2.S,'color',CM(c,:),'linewidth',1.3);hold on;axis tight
